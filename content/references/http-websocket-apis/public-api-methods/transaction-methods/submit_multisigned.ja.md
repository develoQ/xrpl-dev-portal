...
| `Field`     | 型      | Required? | 説明                                                                                                                                |
|:----------- |:------ |:--------- |:--------------------------------------------------------------------------------------------------------------------------------- |
| `tx_json`   | オブジェクト | Yes       | `Signers`からなる配列が指定された[JSONフォーマットのトランザクション](transaction-formats.html)。 成功させるには、署名の重みが[SignerList](signerlist.html)の定数以上でなければなりません。 |
| `fail_hard` | ブール値   | No        | （省略可、デフォルトではfalseです）trueで、かつトランザクションがローカルで失敗する場合は、このトランザクションの再試行や、他のサーバーへのリレーは行わないでください。 The default is `false`.                  |
