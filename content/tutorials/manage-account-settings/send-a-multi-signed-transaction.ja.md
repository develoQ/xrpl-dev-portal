...
署名を並行して収集した場合、すべての署名を含む`tx_json`オブジェクトを手動で作成する必要があります。 すべての`sign_for`応答の`Signers`配列の内容を1つの`Signers`配列に結合します。 結合された`Signers`配列を元のトランザクションのJSON値に追加し、これを\[submit_multisignedメソッド\]\[\]の引数として使用します。
