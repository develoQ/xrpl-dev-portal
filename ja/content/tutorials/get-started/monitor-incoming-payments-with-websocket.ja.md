...
- Try monitoring for transactions of advanced types like [Escrows](escrow.html), [Checks](checks.html), or [Payment Channels](payment-channels.html), and responding to incoming notifications.<!--{# TODO: uncomment when it's ready. - To more robustly handle internet instability, \[Follow a Transaction Chain\](follow-a-transaction-chain.html) to detect if you missed a notification. #}-->## Other Programming Languages
