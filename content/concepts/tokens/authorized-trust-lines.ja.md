...
**ヒント:** 2つのTrustSetトランザクション（ステップ3および4）は、どちらの順序で発生しても構いません。 If the issuer authorizes the trust line first, this creates a trust line with the limit set to 0, and the customer's TrustSet transaction sets the limit on the pre-authorized trust line. (\[TrustSetAuth amendment\]\[\]によって追加されました。 )_
...
- **リファレンス:**
    ...
    - \[AccountSetトランザクション\]\[\]
    - \[TrustSetトランザクション\]\[\]
    - [AccountRootフラグ](accountroot.html#accountroot-flags)
    - [RippleState (トラストライン) フラグ](ripplestate.html#ripplestate-flags)
